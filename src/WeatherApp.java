...
// Retrieve weather data from API - this backend logic fetches the latest weather data
// from the external API and returns it. The GUI will display this data to the user.
public class WeatherApp {
    // Fetch weather data for given location
    public static JSONObject getWeatherData(String locationName){
        // Get location coordinates using the geolocation API
        ...
        try{
            ...
            // Find index of current hour's data
            JSONArray time = (JSONArray) hourly.get("time");
            int index = findIndexOfCurrentTime(time);
    ...
    // Convert the weather code to a human-readable condition
    private static String convertWeatherCode(long weathercode){
        String weatherCondition = "";
        if(weathercode == 0L){
            weatherCondition = "Clear";
